...
namespace of::file::archive
{
	...
	private:
		...
		std::unique_ptr<Requestable> loadInternal(const FileId& modFile)
		{
			...
			{
				auto& logger = of::engine::GetModule<of::logger::Logger>().getLogger("Requestor");
	...
	public:
		...
		**/
		...
		inline common::String editorGetObjectName(const FileId& modFile)
		{
			...
			of::engine::GetModule<of::logger::Logger>().getLogger("Requestor").Warning("Tried to get name of object, but object does not exist!");
			return "";
		}
		...
		inline std::vector<FileId> listAllCurrentLoadedObjects(const ObjectType& objectType) const
		{
			...
			for (it; it != eit; it++)
			{
				...
				{
					of::engine::GetModule<of::logger::Logger>().getLogger("Requestor").Error("Found empty object in requestor" + it->first.operator()());
				}
