...
static uint32_t __glsl_shader_vert_spv[] = {
    0x07230203, 0x00010000, 0x0008000a, 0x0000002e, 0x00000000, 0x00020011, 0x00000001, 0x0006000b, 0x00000001,
...
static uint32_t __glsl_shader_frag_spv[] = {
    0x07230203, 0x00010000, 0x0008000a, 0x0000001e, 0x00000000, 0x00020011, 0x00000001, 0x0006000b, 0x00000001,
...
static const SwChar* properties = "sourceBlend=[SRC_ALPHA,SRC_ALPHA]\n\
...
{
    return ImGui_ImplSwizzle_GetBackendData()->mImGuiFbo;
}
    ...
    common::Unique<swizzle::gfx::DrawCommandTransaction>& dTrans)
{
    ...
    ImGui_ImplVulkan_SetupRenderState(draw_data, dTrans, fb_width, fb_height);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                dTrans->bindMaterial(bd->mShader, *material);
    ...
    dTrans->setScissor(0, 0, (U32)fb_width, (U32)fb_height);
}
    ...
    int fb_height)
{
    ...
    dTrans->bindShader(bd->mShader);
    dTrans->setViewport((U32)fb_width, (U32)fb_height);
    ...
    dTrans->setShaderConstant(bd->mShader, (U8*)&data, sizeof(data));
    if (draw_data->TotalVtxCount > 0)
    {
        dTrans->bindVertexBuffer(bd->mVertexBuffer);
