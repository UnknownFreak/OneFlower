				...
				*/
				if (stats)
				{
					...
					if (inventory)
					{
						...
						{
							of::engine::GetModule<of::logger::OneLogger>().getLogger("Component::Damage").Error("An attack was performed without an equipped main weapon.");
						}
					}
					else
					*/

					{
						...
						of::engine::GetModule<of::logger::OneLogger>().getLogger("Component::Damage").Warning("Attacking entity does not have any inventory and will use \"default \"15.0 weapon damage * level scaling for a total of (" + std::to_string(theDamage) + ")");
						stats->doDamage(theDamage, damageCoef, damageElement, owner);
					}
