...
void SpriteRef::unloadTexture()
{
	if (textureReference)
	{
		...
		if (textureReference->useCount == 0)
			Engine::GetModule<Asset::AssetManager>().textureloader.requestRemovalOfTexture(textureReference->name);
	}
}
...
SpriteRef & SpriteRef::operator=(const SpriteRef & ref)
{
	textureReference = ref.textureReference;
	setTexture(textureReference);
	privRef = this;
	return *this;
}
