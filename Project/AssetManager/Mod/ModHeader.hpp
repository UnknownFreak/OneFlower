...
public:
	...
	void load(Archive& ar)
	{
		...
		{
			ar(modVersion);
			Engine::Get<OneLogger>().Info("Mod [" + name + "] version is: " + modVersion.str());
		}
		else
			Engine::Get<OneLogger>().Info("Current header version does not support loading mod file version (Save file again to apply this feature)");
		ar(mysize);
