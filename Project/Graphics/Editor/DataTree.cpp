...
namespace Graphics::Editor
{
	...
	void DataTree::drawTree(DataTreeItem& item)
	{
		...
		if (itemName.empty())
		{
			...
			//Engine::GetModule<of::logger::OneLogger>().Warning("Object has no display name, using the uuid for it instead.");
		}
		if (itemName == "### ROOT")
		{
		...
		else
		{
			...
			if (item.ptr == nullptr)
			{
				...
				if (ImGui::Button("+", { 18.f, 0 }))
				{
					...
					of::engine::GetModule<of::logger::OneLogger>().Always("Adding new item not yet supported in refactored code");
