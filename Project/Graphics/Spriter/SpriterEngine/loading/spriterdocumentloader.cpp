...
namespace SpriterEngine
{
		...
		while (folderElement->isValid())
		{
			...
			while (fileElement->isValid())
			{
				...
				else
				{
					...
					model->pushBackImageFile(filePath + fileName, pivot, size);
				}
	...
	void SpriterDocumentLoader::getMetaDataFromElement(SpriterFileElementWrapper *parentObjectElement, SpriterModel *model, Entity *entity, Animation *animation, s_int objectId)
	{
	...
	void SpriterDocumentLoader::getVarlinesFromMetaDataElement(SpriterFileElementWrapper *animationElement, Entity *entity, Animation *animation, s_int objectId)
	{
	...
	void SpriterDocumentLoader::getTaglinesFromMetaDataElement(SpriterFileElementWrapper *metaDataElement, SpriterModel *model, Animation *animation, s_int objectId)
	{
		...
		if (att->isValid())
		{
			...
			if (curveType > CURVETYPE_LINEAR)
			{
				...
				while (i < MAX_CONTROL_POINTS)
				{
					...
					if (att->isValid()) {
						controlPoints[i] = (real)att->getIntValue();
					}
				}
			}
		}
	...
	void SpriterDocumentLoader::getRefsFromMainlineKeyElement(SpriterFileElementWrapper *keyElement, Animation *animation, MainlineKey *mainlineKey, SpriteKeyFileInfoObjectIdMap *spriteKeyFileInfoMap, SubEntityKeyInfoMap *subEntityKeyInfoMap, BooleanVector *timelineRedunantKeyedVector)
	{
		...
		while (refElement->isValid())
		{
			...
			size_t objectId = NOT_FOUND;
			size_t parentObjectId = THIS_ENTITY;
			...
			while (att->isValid())
			{
				...
				{
					size_t parentRefIndex = att->getIntValue();
