...
{
	Engine::GetModule<OneLogger>().Warning("Trying to set animation <" + _animationName + "> on a Sprite Sheet Model.", __FILE__, __LINE__);
	Engine::GetModule<OneLogger>().Warning("Currently Sprite Sheet Model only supports one looping animation.", __FILE__, __LINE__);
}

void SpriteSheetModel::setTextureMap(Core::String modOrigin, Core::String textureMapName)
{
	Engine::GetModule<OneLogger>().Warning("Trying to set texure map <" + modOrigin + ", " + textureMapName + "> on a Sprite Sheet Model.", __FILE__, __LINE__);
	Engine::GetModule<OneLogger>().Warning("Sprite Sheet Model does not support texture maps.", __FILE__, __LINE__);
}
