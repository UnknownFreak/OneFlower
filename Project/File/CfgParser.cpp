...
namespace File::Resource::Config
{
	...
	void ConfigParser::load(const Core::String& configFile)
	{
		...
		std::ifstream file(fileName);
		{
			if (file.is_open())
			{
				Engine::GetModule<EngineModule::Logger::OneLogger>().getLogger("ConfigParser").Debug("Begin parsing configuration file", fileName);
				Core::String section = "";
				Core::String line;
				...
				Engine::GetModule<EngineModule::Logger::OneLogger>().getLogger("ConfigParser").Debug("Finished parsing configuration file", fileName);
			}
			else
			{
				Engine::GetModule<EngineModule::Logger::OneLogger>().getLogger("ConfigParser").Error("Unable to open config file: ", fileName);
			}
		}
		file.close();
	}
