				...
				for(it; it != inv->bags[i]->items.end(); ++it)
				{
					if(it->first)
						if(it->first->tag == Item::armor || it->first->tag == Item::weapon)
							items.sprites.push_back(new WindowIcon(it->first->icon,it->first->name,it->first->toToolTipString(),"",Vector2(0,0),std::pair<int,int>(i,ii)));
					ii++;
				}
			}
...
#pragma region swapFromThisWindow
		InventoryComponent* icp = go->GetComponent<InventoryComponent>();
		WindowIcon* _item = (WindowIcon*)items.sprites[items.selectedBag];
				...
				if(Engine::event.type == Engine::event.MouseButtonReleased)
				{
					if(Engine::event.mouseButton.button == sf::Mouse::Button::Left)
					{
						...
						if(icp->bags[_item->index.first]->items[_item->index.second].first)
						{
							...
							std::pair<int,int> pair = ((WindowIcon*)items.sprites[items.selectedBag])->index;

							delete items.sprites[items.selectedBag];
							items.sprites[items.selectedBag] = new WindowIcon(icp->bags[pair.first]->items[pair.second].first->icon,
							...
							if(Engine::GUI.inventory.createdInventory)
							{
								...
								Engine::GUI.inventory.scroll.sprites[ii] = new WindowIcon(icon,
									icp->bags[pair.first]->items[pair.second].first->name,
...
#pragma region swapFromInventoryWindow
		InventoryComponent* icp = go->GetComponent<InventoryComponent>();
		WindowIcon* _item = (WindowIcon*)Engine::GUI.inventory.scroll.sprites[Engine::GUI.inventory.scroll.selectedBag];
